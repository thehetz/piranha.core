...
namespace Piranha.Services
{
    public class SiteService
    {
        ...
        {
            var id = _cache?.Get<Guid?>($"SiteSlug_{slug}");
            Site model = null;
            ...
            {
                model = await _repo.GetBySlug(slug).ConfigureAwait(false);
        ...
        private void OnLoad(Site model)
        {
            if (model != null)
            {
                ...
                if (_cache != null)
                {
                    ...
                    _cache.Set($"SiteSlug_{model.Slug}", model.Id);
                    if (model.IsDefault)
                    {
        ...
        private void RemoveFromCache(Site model)
        {
            if (_cache != null)
            {
                ...
                _cache.Remove($"SiteSlug_{model.Slug}");
